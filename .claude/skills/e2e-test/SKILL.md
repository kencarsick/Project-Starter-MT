---
name: e2e-test
description: Comprehensive end-to-end testing using parallel sub-agents and agent-browser CLI. Launches research agents to understand the app, then tests every user journey with screenshots and database validation. Run after implementation to validate before code review.
...
> "This application doesn't appear to have a browser-accessible frontend. E2E browser testing requires a UI. For backend-only or API testing, use `/verify` with API-specific validation instead."
...
Read `CLAUDE.md` (if it exists) to understand:
...
> Research this codebase thoroughly. Read CLAUDE.md if it exists. Return a structured summary covering:
>
...
>
> 1. **Database type and connection** — what database is used (Postgres, MySQL, SQLite, MongoDB, etc.) and the environment variable name for the connection string (from .env.example)
> 2. **Full schema** — every table/collection, its columns/fields, types, and relationships
...
4. **Take a screenshot** — save to `e2e-screenshots/` organized by journey (e.g., `e2e-screenshots/profile-creation/03-form-submitted.png`)
...
1. Query the database to verify records using the appropriate tool for the DB type:
   - **Postgres:** `psql "$DATABASE_URL" -c "SELECT ..."`
   - **MySQL:** `mysql -u$DB_USER -p$DB_PASS $DB_NAME -e "SELECT ..."`
